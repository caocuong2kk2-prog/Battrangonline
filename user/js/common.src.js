...
// Visual debugger for broken images disabled to allow elegant fallback UI handling
/*
window.addEventListener('error', function(e) {
    if (e.target && e.target.tagName === 'IMG') {
        var src = e.target.src;
        var div = document.createElement('div');
        div.style.cssText = 'color:red;font-size:10px;word-break:break-all;border:1px solid red;padding:2px;background:white;position:absolute;z-index:9999;';
        div.textContent = 'BROKEN: ' + src;
        if (e.target.parentNode) {
            e.target.parentNode.style.position = 'relative';
            e.target.parentNode.insertBefore(div, e.target);
            e.target.style.opacity = '0.1'; // dim it instead of hiding so layout holds
        }
    }
}, true);
*/
...
(function () {
  ...
  async function loadComponents() {
    ...
    // Fetch both in parallel (instantly from sessionStorage if available)
    const pHeader = headerPh ? fetchComponent("header.html") : Promise.resolve(null);
    const pFooter = footerPh ? fetchComponent("footer.html") : Promise.resolve(null);
    ...
    if (headerPh && headerHtml) {
      try {
        ...
        if (newHeader) {
          // Move promo banner out of site-header so it scrolls away naturally, preventing sticky layout loops
          const promoBanner = document.getElementById('top-promo-banner');
          if (promoBanner && newHeader.parentNode) {
            newHeader.parentNode.insertBefore(promoBanner, newHeader);
          }
        ...
        headerPh.remove();
        if (typeof populateHeaderMegaMenu === 'function') {
          populateHeaderMegaMenu();
        }
...
function populateHeaderMegaMenu() {
    ...
    window.PhucGiaTienAPI.getFilters().then(function(filters) {
      ...
      var megaMenuEl = container.closest('.mega-menu');
      if (megaMenuEl) {
        megaMenuEl.classList.add('has-content');
      }
    }).catch(function(err) {
  ...
  function initFooter() {
    ...
    const contactToggle = document.getElementById('quick-contact-toggle');
    const contactWidget = document.querySelector('.quick-contact-widget');
    ...
    if (btn) {
      // ── Ẩn/hiện: Sử dụng IntersectionObserver để chống layout thrashing ──
  ...
  window.handleImageFallback = function(img) {
    if (img.dataset.fallbackImages) {
      try {
        var urls = JSON.parse(img.dataset.fallbackImages);
        var idx = parseInt(img.dataset.fallbackIndex, 10) || 0;
        idx++;
        if (idx < urls.length) {
          img.dataset.fallbackIndex = idx;
          var nextImgSrc = urls[idx];
          // Determine basepath
          var pn = window.location.pathname.toLowerCase();
          var basePath = (pn === '/user' || pn.startsWith('/user/')) ? '/user/' : '/';

          if (typeof window.resolveImgUrl === 'function') {
            nextImgSrc = window.resolveImgUrl(nextImgSrc, nextImgSrc, 550);
          } else if (!nextImgSrc.startsWith('http') && !nextImgSrc.startsWith('/') && !nextImgSrc.startsWith('data:')) {
            nextImgSrc = basePath + nextImgSrc;
          }
  ...
  window.buildProductCard = function(p, i, isEager) {
    ...
    article.className = 'product-card' + (isEager ? '' : ' reveal');
    ...
    article.innerHTML =
      ...
      (isLocalVid
        ...
        : '<img class="product-card__img" src="' + imgSrc + '"' + srcsetHtml + ' alt="' + pName.replace(/"/g, '&quot;') + '" ' + (isEager ? 'loading="eager"' : 'loading="lazy"') + ' decoding="async" data-fallback-images=\'' + JSON.stringify(allImages).replace(/'/g, '&apos;').replace(/"/g, '&quot;') + '\' data-fallback-index="0" onerror="window.handleImageFallback(this)">') +
      '</div>' +
  ...
  function resolveImgUrl(url, defaultUrl, width, quality, height) {
    ...
    // If it's a relative path to assets, make it absolute so we can proxy it if needed
    if (resolved.startsWith('assets/')) {
        resolved = '/' + resolved; // make root relative
    }
    ...
    if (resolved.startsWith('/') && !resolved.startsWith('//')) {
      ...
      if (!dynamicBase) {
          dynamicBase = window.location.origin;
      }
      resolved = dynamicBase + resolved;
    }
    
    // Now resolved is an absolute URL
    var isLocalOrProd = resolved.includes('battrangonline.vn') || resolved.includes(window.location.hostname) || resolved.includes('localhost');
    ...
    if (resolved && !resolved.startsWith('data:') && (isLocalOrProd || isLocalUpload)) {
      ...
      if (resolved.startsWith('http')) {
         try {
             ...
             if (u.hostname.includes('trycloudflare.com') || u.hostname === 'localhost' || u.hostname === '127.0.0.1') {
      ...
      var proxyEndpoint = proxyApiBase + '/imageproxy?url=' + encodeURIComponent(proxyUrl) + '&w=' + w + '&q=' + q;
      if (height) proxyEndpoint += '&h=' + height;
  ...
  window.resolveImgUrl = resolveImgUrl;
  ...
  function applyDynamicConfig() {
    ...
    document.querySelectorAll('.js-config-logo').forEach(function (el) {
      var logoUrl = config.logoUrl ? resolveImgUrl(config.logoUrl, '', 100) : 'assets/images/logo.png';
    ...
    if (homeText) {
      document.querySelectorAll('.js-config-home-story-text').forEach(function (el) {
        var current = el.innerHTML.replace(/\s+/g, '');
        var next = homeText.replace(/\s+/g, '');
        if (current !== next) {
          el.innerHTML = homeText;
        }
      });
    }
