      ...
      tbody.innerHTML = slice.map(function (o, idx) {
        var stt = (currentPage - 1) * pageSize + idx + 1;
          ...
          if (o.isCancelRequested) {
              ...
              cancelBadge = '<br><span style="display:inline-block; margin-top:4px; font-size:10px; background-color:#ffedD5; color:#c2410c; padding:2px 6px; border-radius:4px; font-weight:600;"><i class="fas fa-exclamation-circle"></i> Khách xin hủy' + (reqTime ? ' (' + reqTime + ')' : '') + '</span>';
          ...
          var customerNoteHtml = o.customerNote ? '<div class="customer-note-badge" title="' + escapeHTML(o.customerNote) + '" style="display:inline-flex;align-items:center;gap:4px;padding:2px 8px;background:#fffbeb;border:1px solid #fde68a;border-radius:4px;color:#b45309;font-size:10.5px;font-weight:600;cursor:help;max-width:max-content;box-shadow:0 1px 2px rgba(0,0,0,0.02);"><i class="fas fa-comment-dots" style="color:#d97706;font-size:10px;"></i> <span>Xem ghi chú</span></div>' : '—';
  ...
  function updateOrderStatus(id, status) {
    ...
    AdminData.orders.updateStatus(id, status).then(function () {
      ...
      if (idx >= 0) {
          ...
          orders[idx].isCancelRequested = false;
      }
    ...
    if(window.adminPrompt) {
      adminPrompt('Bạn muốn từ chối yêu cầu hủy của khách đối với đơn #' + id + '?\nVui lòng nhập lý do từ chối (bắt buộc):', '', function(reason) {
        ...
        AdminData.orders.rejectCancel(id, reason).then(function () {
          ...
          var idx = orders.findIndex(function (o) { return o.id === id; });
          if (idx >= 0) { orders[idx].isCancelRequested = false; orders[idx].cancelReason = "Bị từ chối: " + reason; }
          renderStatusTabs();
          renderTable();
          closeModal('orderDetailModal');
        }).catch(function (err) {
      ...
      AdminData.orders.rejectCancel(id, "Shop từ chối").then(function () {
        adminToast('Đã từ chối yêu cầu hủy đơn.', 'success');
        var idx = orders.findIndex(function (o) { return o.id === id; });
        if (idx >= 0) { orders[idx].isCancelRequested = false; }
        renderStatusTabs();
        renderTable();
        closeModal('orderDetailModal');
      }).catch(function (err) {
